...
folder_Data_description = 'Data_description'
if not os.path.exists(folder_Data_description):
    os.makedirs(folder_Data_description)
...
data = preprocess_function.delete_NotDeath(data, folder_Data_description)
data.to_excel((os.path.join(folder_name, '02_delete_NotDeath.xlsx')), index=False)

# “religious_beliefs”中的缺值歸類至民間信仰裡面
data = preprocess_function.religion_replace_na_with_folk(data, folder_Data_description)
...
data = preprocess_function.Combine_DNRtiming_and_IsSignDNR_Into1Col(data, folder_Data_description)
...
data = preprocess_function.medicine_morphine(data, folder_Data_description)
data.to_excel((os.path.join(folder_name, '05_medicine_morphine.xlsx')), index=False)

# 收案日期改為春節或非春節
data = preprocess_function.StartDay_To_lunar_New_Year(data, folder_Data_description)
...
# most_frequent_diagnosis = diagnosis_counts.idxmax()
# print(diagnosis_counts)

# 疾病診斷處理(轉碼)
data = preprocess_function.DiseaseDiagnosis_Transcoding(data, folder_Data_description)
